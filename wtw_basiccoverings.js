...
WTWJS.prototype.addWebsiteCovering = function(zmoldname, zmolddef, websiteSrc, websiteWidth, websiteHeight) {
    var zcovering;
    try {
        var zopacity = 1;
        ...
        var website = new BABYLON.GUI.Rectangle("website");
        website.width = websiteWidth;
        website.height = websiteHeight;
        website.background = "white";
        advancedTexture.addControl(website);

        var websiteHTML = new BABYLON.GUI.TextBlock();
        websiteHTML.text = `<iframe src="${websiteSrc}" width="${websiteWidth}" height="${websiteHeight}" frameborder="0" allowfullscreen></iframe>`;
        websiteHTML.color = "black";
        websiteHTML.fontSize = 12;
        websiteHTML.textHorizontalAlignment = BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_CENTER;
        websiteHTML.textVerticalAlignment = BABYLON.GUI.Control.VERTICAL_ALIGNMENT_CENTER;
        website.addControl(websiteHTML);

        zcovering.diffuseTexture = advancedTexture;
    } catch (ex) {
        WTW.log('core-scripts-coverings-basiccoverings\r\n addWebsiteCovering=' + ex.message);
    }
    return zcovering;
}
